...
namespace Ayttekao.Unity.Editor
{
    public class PopupWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            doNotShowAgain = GUILayout.Toggle(doNotShowAgain, "Do not show this popup again. <size=8>(All popups can be enabled again by going to \"Edit/Ayttekao - Enable all popups again\")</size>", new GUIStyle(EditorStyles.toggle) {richText = true});
        ...
        public static void Open(IPopup popup)
        {
            ...
            window.minSize = size;
            window.maxSize = size;
            window.titleContent = new GUIContent($"Ayttekao - {popup.Title}");
            window.popup = popup;
            window.ShowUtility();
